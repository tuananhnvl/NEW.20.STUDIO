...
export default class Cursor {
  constructor(el) {
    ...
    this.Item = document.querySelectorAll(".item-services-active");
    this.Hero = document.querySelector("#item-services");
  ...
  setupMouse() {
    console.log('setupMouse run')
    window.addEventListener("mousemove", (ev) => (mouse = getMousePos(ev)));
   //console.log(mouse)
  }
  onScaleMouse() {
    console.log('onScaleMouse run')
  ...
  setVideo(el) {
    ...
    if (video.id == src) {
        if(video.id == 'websites') {
            gsap.set(video, { zIndex: 4, opacity: 1});
            gsap.set(video.parentElement, { transform:'translateX(-500px)'});
        }
 ...
 //   console.log(this.cursorConfigs.x.current,this.cursorConfigs.y.current)
    // lerp
  ...
  //  console.log(`${this.cursorConfigs.x.previous},${this.cursorConfigs.y.previous}`)
  //console.log(this.boundsCursorDemo)
 //this.boundsCursorDemo.y = `${this.cursorConfigs.y.previous}`
 // this.Cursor.style.left = `calc(${this.cursorConfigs.y.previous}px +175px)`
   // this.DemoCursor.style.transform = `translateX(${this.cursorConfigs.x.previous}px) translateY(${this.cursorConfigs.y.previous}px)`;
