...
import { useIntersect,Html,OrbitControls,Loader } from '@react-three/drei'
...
function Item({data,texture, ...props }) {
 ...
 function handleClick() {
  ...
  localStorage.setItem('lockUframe', 1) 
  grRef.current.position.z = 3
  ...
  return (
    <group ref={grRef} {...props} onClick={handleClick}>
...
useEffect(() => {
  ...
  useFrame((state, delta) => {
   // console.log(localStorage.getItem('loadTex-complete'))
     ...
     {/*  <mesh ref={planeRefClone} position={[0.0,0.0,0.0]}>
      ...
      <group ref={groupItemRef}   {...bindDrag()}>
        {textures.map((texture, index) => (
          <Item data={index} key={index} texture={texture} />
        ))}
      </group>
    </group>
  ...
  return (
    <section id='wrapper-scene' style={{ height: '100vh', width: '100vw' }}>
      <Canvas
        ...
        camera={{ position: [0, 0,4.5], zoom: 1, up: [0, 0, 0], far: 10 }}
      >
