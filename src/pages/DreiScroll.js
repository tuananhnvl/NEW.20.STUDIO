...
import { useIntersect, shaderMaterial,Html } from '@react-three/drei'
...
function Items() {
    ...
    const loadTextures = () => {
      ...
      return Promise.all(textures.map((texture) => {
        return new Promise((resolve) => {
          ...
          let width = texture.source.data.naturalWidth;
          let height = texture.source.data.naturalHeight;
          let ratio = { w: width / 1000, h: height / 1000 };
          mapRatio.push(ratio);
          resolve(ratio);
  ...
  useEffect(() => {
    ...
    loadTexture()
      .then((mapPosGeneral) => {
        console.log('==>>  calculateMapPosGeneral')
  ...
  }, [loadTexture, mapPosGeneral, totalmarginW]);
  ...
  useFrame((state, delta) => {
    ...
    if( localStorage.getItem('unBlock-Uframe') == 1) {
      countTime += delta / 2
